...
class AstronInternalRepository(ConnectionRepository):
    ...
    def sendActivate(self, doId, parentId, zoneId, dclass=None, fields=None):
        ...
        if dclass and fields:
            ...
            dg.addUint32(0)
            self.send(dg)
            # DEFAULTS_OTHER isn't implemented yet, so we chase it with a UPDATE_FIELD_MULTIPLE
            dg = PyDatagram()
            dg.addServerHeader(doId, self.ourChannel, STATESERVER_OBJECT_UPDATE_FIELD_MULTIPLE)
            dg.addUint32(doId)
